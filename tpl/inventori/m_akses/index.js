app.controller("aksesCtrl", function ($scope, Data, $rootScope, $state, UserService, $uibModal) {
    /**
     * Inialisasi
     */
    var tableStateRef = {};
    ...
    $scope.checkAll = function (param, is) {
        $('.' + param + ' input[type="checkbox"]').each(function (val) {
            var arr = $(this).attr('ng-model').split(".");
            $scope.form.akses[arr[arr.length - 1]] = is;
        })
    }
});