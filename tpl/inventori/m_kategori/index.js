app.controller('kategoriCtrl', function ($scope, Data, $rootScope, $uibModal, Upload) {
    var tableStateRef;
    var control_link = "m_kategori";
    var master = 'Master Kategori';
    $scope.formTitle = '';
    $scope.displayed = [];
    $scope.base_url = '';
    $scope.is_edit = false;
    ...
    Data.get('acc/m_akun/akunDetail').then(function (data) {
        $scope.listAkunPembelian = data.data.list;
        $scope.listAkunPenjualan = data.data.list;
        $scope.listAkunHPP = data.data.list;
    });

    $scope.getParent = function () {
        Data.get(control_link + '/index', {filter: {'inv_m_barang.id': 0}}).then(function (data) {
...
//    $scope.fillParent = function () {
//        if ($scope.form.is_parent) {
//            $scope.form.parent = undefined;
//        } else {
//            if ($scope.parent.length > 0 && $scope.is_create) {
//                $scope.form.parent = $scope.parent[0];
//            }
//        }
//    }

    $scope.resetFilter = function (form) {
        $scope.form[form] = undefined;
    }
    ...
    $scope.create = function () {
        ...
        $scope.form = {};
        $scope.form.is_parent = true;
        $scope.form.is_dijual = 'tidak';
        $scope.form.jenis_barang = 'pembelian';
        $scope.getParent();
    };
    /** update */
