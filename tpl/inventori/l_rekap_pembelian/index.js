app.controller('l_rekap_pembelian', function ($scope, Data, $rootScope, $uibModal, Upload) {
    var control_link = "l_rekap_pembelian";
    $scope.form = {};
    $scope.form.bulan_awal = new Date();
    $scope.form.bulan_akhir = new Date();
    $scope.form.is_ppn = 1;
    /**
     * Ambil list lokasi
     */
    Data.get('acc/m_lokasi/getLokasi').then(function (response) {
        ...
        $scope.form.lokasi = response.data.list[0];
    });
    ...
    Data.get('m_kategori/index', {filter: {"inv_m_kategori.is_deleted": 0, "inv_m_kategori.is_parent": 0}}).then(function (response) {
        ...
        if (response.data.list.length > 0) {
          angular.forEach($scope.listKategori, function (val, key) {
            if(val.id == 3){ //  auto select barang dagangan
              $scope.form.kategori = val;
            }
          });
        }
    })
        ...
        if (is_export == 0 && is_print == 0) {
            Data.get(control_link + '/laporan', param).then(function (response) {
                if (response.status_code == 200) {
                    ...
                    $scope.allTotal       = response.data.totalSemua;
...
//                console.log(response)
                window.open(response.data.base_url + "api/l_rekap_pembelian/laporan?" + $.param(param), "_blank");
            });
        }
    };
