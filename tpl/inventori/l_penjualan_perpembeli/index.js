app.controller('l_penjualan_perpembeli', function ($scope, Data, $rootScope, $uibModal, Upload) {
     ...
     */
    $scope.tipe = 'buku';
    $scope.view = function (is_export, is_print, tipe = 'buku') {
        $scope.tipe = tipe;
        ...
        if (is_export == 0 && is_print == 0) {
            Data.get(control_link + '/laporan', param).then(function (response) {
                if (response.status_code == 200) {
                    ...
                    $scope.data.tgl_mulai = response.data.tgl_mulai;
                    $scope.data.tgl_selesai = response.data.tgl_selesai;
