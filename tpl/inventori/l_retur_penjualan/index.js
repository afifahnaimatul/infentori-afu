app.controller('l_retur_penjualan', function ($scope, Data, $rootScope, $uibModal, Upload) {
    var control_link = "l_retur_penjualan";
    $scope.form = {};
    $scope.form.tanggal = {
    ...
    // Data.get('l_faktur_sederhana/listbarang').then(function(response) {
    ...
    $scope.cariBarang = function ($query) {
        ...
        if ($query.length >= 3) {
            Data.get('l_retur_penjualan/listbarang', {
                'nama': $query
    ...
    $scope.view = function (is_export, is_print) {
        ...
        var param = {
            export: is_export,
            print: is_print,
            acc_m_lokasi_id: $scope.form.m_lokasi_id != undefined ? $scope.form.m_lokasi_id.id : null,
            ...
            tanggal: moment($scope.form.tanggal).format('YYYY-MM'),
            show_kartu: true,
        };
...
//                console.log(response)
                window.open(response.data.base_url + "api/l_retur_penjualan/laporan?" + $.param(param), "_blank");
            });
        }
    };
    ...
    //     Data.post('l_faktur_sederhana/laporan', form).then(function(response) {
    //         if (response.status_code == 200) {
    ...
    //     console.log($scope.getBarang);
    // });
    // Data.get("l_faktur_sederhana/listcabang").then(function(response) {
    ...
    //     Data.get('l_faktur_sederhana/listbarang?produk_id=' + $stateParams.produk).then(function(response) {
    ...
    $scope.exportData = function(clases) {
        ...
        saveAs(blob, "Laporan-Retur-Penjualan.xls");
    };
});
