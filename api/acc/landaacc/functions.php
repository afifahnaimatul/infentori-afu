...
function generateNoTransaksi($type, $unker, $preffix = null, $bulan = null, $tahun = null)
{
    ...
    if ($type == 'penerimaan') {
        ...
        if ($custom->reset_kode == 'bulanan') {
            if (!empty($preffix)) {
                $cek = $db->find("select no_transaksi from acc_jurnal WHERE MONTH(tanggal) = '" . $bulan . "' AND YEAR(tanggal) = '" . $tahun . "' AND no_transaksi LIKE '{$preffix}%' order by no_urut desc");
            } else {
                $cek = $db->find("select no_transaksi from acc_jurnal WHERE MONTH(tanggal) = '" . $bulan . "' AND YEAR(tanggal) = '" . $tahun . "' order by no_urut desc");
            }
        } else {
            if (!empty($preffix)) {
                $cek = $db->find("select no_transaksi from acc_jurnal WHERE YEAR(tanggal) = '" . $tahun . "' AND no_transaksi LIKE '{$preffix}%' order by no_urut desc");
            } else {
                $cek = $db->find("select no_transaksi from acc_jurnal WHERE YEAR(tanggal) = '" . $tahun . "' order by no_urut desc");
            }
        }
        $urut = (empty($cek)) ? 1 : ((int) substr($cek->no_transaksi, $custom->digit_kode)) + 1;
        $no_urut = substr('00000' . $urut, $custom->digit_kode);
