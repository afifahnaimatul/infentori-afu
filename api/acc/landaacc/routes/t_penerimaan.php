...
$app->get('/acc/t_penerimaan/getDetail', function ($request, $response) {
    ...
    $getListInduk = getAkunTipe();
    foreach ($models as $key => $val) {
        $val->m_akun_id = [
          "id"          => $val->m_akun_id,
          "kode"        => $val->kodeAkun,
          "nama"        => $val->namaAkun,
          "indukan"     => !empty($getListInduk[$val->parent_id]) ? $getListInduk[$val->parent_id] : [],
        ];
...
$app->get('/acc/t_penerimaan/setAkunDimuka', function ($request, $response) {
    $params = $request->getParams();
    $db = $this->db;

    // Tambahkan akun Pendapatan dimuka
    $getListInduk = getAkunTipe();
    $akunPDM = $db->select("acc_m_akun.*")
    ->from("acc_m_akun_peta")
    ->leftJoin("acc_m_akun", "acc_m_akun_peta.m_akun_id = acc_m_akun.id")
    ->where("acc_m_akun_peta.type", "=", 'Pendapatan Dimuka')
    ->find();

    if( !empty($akunPDM) ){
      $akunPDM->indukan = !empty($getListInduk[$akunPDM->parent_id]) ? $getListInduk[$akunPDM->parent_id] : [];
    }

    return successResponse($response, ['list' => $akunPDM]);
});
